...
public class WorkmenWageRateLoader
  ...
  implements IPluginAppCommonData
{
  ...
  ArrayList list;
  HashMap<ObjectIdLong, Double> idMap;
  
  public WorkmenWageRateLoader(TableDataHandler tbHandler, SummaryViewDetail svDetail, ArrayList ccDetail, int vType, int col, ICommonData data) {
  ...
  protected ColumnData getData() {
    ...
    ArrayList payCodeLoader = m_data.getPayCodeTotalLoader();
    ...
    stmt = new SQLStatement(3, "business.cms.SummaryViewSqlStringConstant.WORKMENWAGE", sqlSubstitutionArrayList);
    ...
    for (DataStore ds = stmt.getDataStore(); ds.nextRow();)
    {
      ...
      rowData[1] = Double.valueOf(ds.getDouble(2).doubleValue() + ds.getDouble(3).doubleValue() + ds.getDouble(4).doubleValue());
      idMap.put(ds.getObjectIdLong(1), Double.valueOf(ds.getDouble(2).doubleValue() + ds.getDouble(3).doubleValue() + ds.getDouble(4).doubleValue()));
      list.add(rowData);
    }
