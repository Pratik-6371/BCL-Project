...
function reset(){
    ...
    for(var i = 0; i < elements.length; i++){
    	...
    	if(elements[i].name == 'value(empcode)'){
    		elements[i].value = "";
    	}
    	else if(elements[i].name == 'value(empname)'){
    		elements[i].value = "";
    	}
    }
    document.forms.healthInformationWorkmenListForm.doAction ('<%= KronosProperties.get("cms.action.refresh").replaceAll("'","\\\\'") %>');
}
</script>

<bean:define id="cmsTable" name="org.apache.struts.taglib.html.BEAN" property="valueAsList(cmsWorkmen)" />
<bean:define id="availUnitNames" name="org.apache.struts.taglib.html.BEAN" property="value(availUnitNames)"/>
	  ...
	  <select id="selectedPE" name="value(unitId)"> 
		...
		>
          <c:out value="${principalEmployer.unitName}"/>
						...
						<td><th><label for="khtmlNameInput"><span></span>
	 ...
	 <c:forEach var = "item" items = "${cmsTable}" varStatus="status">					
		...
		<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
			...
			<c:if test="${item.eCode == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.id}" />);document.forms.healthInformationWorkmenListForm.doAction('<%= KronosProperties.get("cms.action.edit").replaceAll("'","\\\\'") %>')"><c:out value='${item.eCode}'/></a>
			</td>
			...
			<c:if test="${item.firstname == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.id}" />);document.forms.healthInformationWorkmenListForm.doAction('<%= KronosProperties.get("cms.action.edit").replaceAll("'","\\\\'") %>')"><c:out value='${item.firstname}'/></a>
