...
<bean:define id="availUnitNames" name="contractorlistForm" property="value(availUnitNames)"/>
<div class="Panel first-child">
  <table>
	...
	<fmt:message key="label.ccode" />
	  <html:text property="value(ctcode)" styleId="khtmlNameInput"
	  								size="20"
									maxlength="20"  />
	</td>
	<td  class="last-child">
	 ...
	 <c:forEach var = "item" items = "${webServiceConfigTable}" varStatus="status">					
		...
		<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
			...
			<c:if test="${item.vendorcode == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.id}" />);document.forms.contractorlistForm.doAction('<%= KronosProperties.get("cms.action.edit").replaceAll("'","\\\\'") %>')"><c:out value='${item.vendorcode}'/></a>
			...
			<c:if test="${item.contractorname == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.id}" />);document.forms.contractorlistForm.doAction('<%= KronosProperties.get("cms.action.edit").replaceAll("'","\\\\'") %>')"><c:out value='${item.contractorname}'/></a>
