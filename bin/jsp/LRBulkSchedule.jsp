...
function reset(){
    ...
    document.forms.lrbulkScheduleForm.doAction ('<%= KronosProperties.get("cms.action.laborreq.refresh").replaceAll("'","\\\\'") %>');
}
...
<bean:define id="fromDtm1" name="lrbulkScheduleForm" property="value(fromDtm1)"/>
<bean:define id="toDtm1" name="lrbulkScheduleForm" property="value(toDtm1)"/>


<%-- <bean:define id="availUnitNames" name="laborreqbulkForm" property="value(availUnitNames)"/> --%>
<div class="Panel first-child">
  <table>
    <tr>
	<td  class="last-child">
	<fmt:message key="label.wkNum" />
	  <html:text property="value(wkNumber)"
									styleId="khtmlNameInput" size="20"
									maxlength="20"  />
	
						
						<fmt:message key="label.from" />
						<input id="value(fromDtm1)"
										name="value(fromDtm1)"
...
<v3lists:sort
	var			= "bulkScheduleSorter"
	items		= "webServiceConfigTable"
	property	= "state(cms_bk_state)"
	default		= "pageNum">
<v3lists:paginate
	var			= "paginate1"
	...
	<thead>
		<tr>
			...
			<tiles:insert name = "sort-ui" >
				<tiles:put name = "sorter" beanName	= "bulkScheduleSorter" />
				<tiles:put name = "property" value	= "state(cms_bk_state)" />
	 ...
	 <c:forEach var = "item" items = "${webServiceConfigTable}" varStatus="status">					
		...
		<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
			...
			<c:if test="${item.pageNum == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.pageId}" />);document.forms.lrbulkScheduleForm.doAction('<%= KronosProperties.get("cms.action.laborreq.schedule").replaceAll("'","\\\\'") %>')"><c:out value='${item.pageDispNum}'/></a>
