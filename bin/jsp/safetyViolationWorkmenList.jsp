...
function reset(){
    ...
    document.forms.safetyWorkmenListForm.doAction ('<%= KronosProperties.get("cms.action.refresh").replaceAll("'","\\\\'") %>');
}
</script>
	 ...
	 <c:forEach var = "item" items = "${cmsTable}" varStatus="status">					
		...
		<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
			...
			<c:if test="${item.eCode == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.id}" />);document.forms.safetyViolationWorkmenListForm.doAction('<%= KronosProperties.get("cms.action.edit").replaceAll("'","\\\\'") %>')"><c:out value='${item.eCode}'/></a>
			</td>
			...
			<c:if test="${item.firstname == null}">
				...
				<a href="javascript:setSelectedItem(<c:out value="${item.id}" />);document.forms.safetyViolationWorkmenListForm.doAction('<%= KronosProperties.get("cms.action.edit").replaceAll("'","\\\\'") %>')"><c:out value='${item.firstname}'/></a>
