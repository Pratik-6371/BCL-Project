...
<%-- <script type="text/javascript"
	src="<%=URLUtils.clientUrlStatic("/applications/struts/scripts/demo.js")%>"></script> --%>
...
function getRefreshData() 
{
   ...
   if(checkUnsavedData())
   {
	   document.forms.workmenDetailForm.doAction ('<%= KronosProperties.get("cms.action.workmen.detail.safety.violation").replaceAll("'","\\\\'") %>');
...
function getExp()
{
	...
	var dateObjectTime = dateObject.getTime();
...
function getAge()
{
	...
	var underAge = <%= KronosProperties.get("cms.employee.under.age").replaceAll("'","\\\\'") %>;
	var overAge = <%= KronosProperties.get("cms.employee.over.age").replaceAll("'","\\\\'") %>;
	// month is 0-based, that's why we need dataParts[1] - 1
	var dateObject = new Date(+dateParts[2], dateParts[1] - 1, +dateParts[0]); 
	var dateObjectTime = dateObject.getTime();
	var diff_ms = Date.now() - dateObject.getTime();
	...
	var todayTime = today.getTime();
	 ...
	 if(dateObjectTime >= todayTime) {
		alert("Date of Birth cannot be Today Or After Today's Date");
		document.getElementById("value(dob)").value = "";
		...
		alert("Age Cannot be more than 58");
		document.getElementById("value(dob)").value = "";
		document.getElementById("age").value = "";
		return false;
	}
...
function validateDOT()
{
	...
	function validate(){
		...
		var proflic4 = document.getElementById('proflic4').value;//UAN
		var empmobileno = document.getElementById('empmobileno').value;
		var pfExempt = document.getElementById('pfExempt');
		...
		var vehicleRent = document.getElementById('vehicleRent').value;
		var quarterRent = document.getElementById('quarterRent').value;
		...
		else if(dateDobTime >= dateDojTime) {
			alert("Date of Birth cannot be Same or After Date of Joining");
			return false;
			}
		...
		return false;
			...
			if(item_serviceItemId === "-1"){  
			 ...
			 if(esic != null && esic !== ""){
					...
					if(isNaN(esic)){
						console.log(esic);
						alert("ESI Should be Numeric!!");
						return false;
					}
					else if(esic.length != 10){
						
						alert("ESI Should be 10 Digits!!");
						return false;
					}
				}
		  
		
		  if(proflic4 != null && proflic4 !== ""){
			
			if(isNaN(proflic4)){
				
				alert("Universal Account Number Should be Numeric!!");
				return false;
			}
			else if(proflic4.length != 12){
				alert("Universal Account Number Should be 12 Digits!!");
				return false;
			}
		} 	
		  if(empmobileno != null && empmobileno !== ""){
				
				if(isNaN(empmobileno)){
		  ...
		  if(isNaN(vehicleRent)){
			  alert("Vehicle Rent Should be Numeric!!");
				return false;
		  }
		  if(isNaN(quarterRent)){
			  alert("Quarter Rent Should be Numeric!!");
				return false;
		  }
		  if(shift === "-1"){
	...
	function sameAddress(){
		...
		var checkboxState = document.getElementById("address").checked;
		...
		var pStateId = document.getElementById("value(pStateId)").value;
		
		if(checkboxState == true){
			document.getElementById("permVillage").value = pVillage;
			document.getElementById("permVillage").readOnly = true;
			document.getElementById("permTaluka").value = pTaluka;
			document.getElementById("permTaluka").readOnly = true;
			document.getElementById("permDistrict").value = pDistrict;
			document.getElementById("permDistrict").readOnly = true;
			document.getElementById("value(permStateId)").value = pStateId;
			document.getElementById("value(permStateId)").readOnly = true;
		}
		else{
			document.getElementById("permVillage").readOnly = false;
			document.getElementById("permTaluka").readOnly = false;
			document.getElementById("permDistrict").readOnly = false;
			document.getElementById("value(permStateId)").readOnly = false;
		}
	}
...
<bean:define id="imageContentValue" name="workmenDetailForm" property="value(imageContent)" />
<bean:define id="dob" name="workmenDetailForm" property="value(dob)" />
<bean:define id="availContrCodes" name="workmenDetailForm" property="value(availContrCodes)" />
<bean:define id="availDepartmentNames"  name="workmenDetailForm" property="value(availDepartmentNames)"/>
<bean:define id="availSectionNames" name="workmenDetailForm" property="value(availSectionNames)"/>
<bean:define id="empTypeValue" name="workmenDetailForm" property="value(empTypeValue)" />
<bean:define id="supplyTypeValue" name="workmenDetailForm" property="value(supplyType)" />
...
<bean:define id="availDeviceGroupNames" name="workmenDetailForm" property="value(availDeviceGroupNames)" />
<bean:define id="gender" name="workmenDetailForm" property="value(gender)" />
<%-- <bean:define id="prevExp" name="workmenDetailForm" property="value(prevExp)" /> --%>
<%-- <bean:define id="prevOrg" name="workmenDetailForm" property="value(prevOrg)" /> --%>
<bean:define id="std" name="workmenDetailForm" property="value(std)" />
<bean:define id="availUnitNames" name="workmenDetailForm" property="value(availUnitNames)"/>
<html:hidden property="value(status)" styleId="khtmlWsConfigId" />
<bean:define id="availWorkorder" name="workmenDetailForm" property="value(availWorkorder)"/>
<bean:define id="availWorkorderLine" name="workmenDetailForm" property="value(availWorkorderLine)"/>
<bean:define id="effdt" name="workmenDetailForm" property="value(effdt)" />
<bean:define id="bloodgroup" name="workmenDetailForm" property="value(bloodgroup)" />
<bean:define id="wageEffdt" name="workmenDetailForm" property="value(wageEffdt)" />
<bean:define id="vehicleType" name="workmenDetailForm" property="value(vehicleType)" />
<bean:define id="shift" name="workmenDetailForm" property="value(shift)" />
					...
					<table>
						...
						<tr>
							...
							<td><html:text property="value(eCode)"
									styleId="khtmlNameInput" onchange="setDataChanged();" 
					...
					{	%>
						<tr>
							...
							<td><html:text property="value(status)"
									styleId="khtmlNameInput" onchange="setDataChanged();" size="10"
					...
					<%} %>
						...
						<tr>
							...
							<%-- <td><html:text property="value(gender)"
									...
									maxlength="50" /></td> --%>
							<td>
								<select id="selectedFunc" name="value(gender)"
									onchange="setDataChanged()">
									<option value="-1"
										<c:if test="${gender == '-1'}">selected = "selected"</c:if>>Select Gender</option>
									<option value="Male"
										<c:if test="${gender == 'Male'}">selected = "selected"</c:if>>Male</option>
									<option value="Female"
										<c:if test="${gender == 'Female'}">selected = "selected"</c:if>>Female</option>	
								</select>
							</td>
						</tr>
						...
						<tr>
							...
							<td><input id="value(dob)" name="value(dob)"
								onchange="getAge(); setDataChanged();" type=text size=20
								value="<c:out value="${dob}"/>"> </input>
						...
						<td><html:text property="value(age)" styleId="age" readonly="true"
...
ObjectIdLong workorderLineId = (ObjectIdLong) request.getAttribute("workorderLineId");%>
						...
						<tr>
							...
							<td>							
								<select id="value(ccode)" name="value(ccode)" onchange="getRefreshData();" style="width: 140px">
									...
									<c:forEach var="ccode" items="${availContrCodes}">
										...
										<c:if test="${ccode.contractorid == contrid}">selected</c:if>>
											  <c:out value="${ccode.contractorName}" /> - <c:out value="${ccode.vendorCode}" />
										</option>
									</c:forEach>
								</select>
							</td>
						...
						<%-- <tr>
							...
							<td>
								  <select id="itemNumId" name="value(itemNum)" onchange="setDataChanged();"> 
							        ...
							        <option value="<c:out value="${lineItem.wkLineId}"/>" 
						...
						<tr>
							...
							<td>
								  <select id="serviceLineItemId" name="value(serviceLineItem)" onchange="setDataChanged();"> 
								  	<option value="-1">Select Service Line Item</option>
								     <c:forEach var="serviceLineItem" items="${availWorkorderLine}"> 
							        <option value="<c:out value="${serviceLineItem.wkLineId}"/>" 
						...
						<tr style="display:<%=pfEx%>">
							...
							<td><html:text property="value(pfaccno)"
									styleId="pfaccno" onchange="setDataChanged();" size="40" 
									maxlength="50" /></td>
						</tr>
						...
						<tr>
							...
							<th><html:text property="value(proflic4)"
									styleId="proflic4" onchange="setDataChanged();" size="12"
									maxlength="12" /></td>
						</tr>
						...
						<tr>
							...
							<td>
								<select id="empType" name="value(empType)"
									...
									<option value="-1"
										<c:if test="${empTypeValue == '-1'}">selected = "selected"</c:if>>Select Employee Type</option>
									<option value="WB"
						...
						<tr>
							...
							<td>
								<select id="selectedFunc" name="value(supplyType)"
									onchange="setDataChanged()">
									<option value="-1"
										<c:if test="${supplyTypeValue == '-1'}">selected = "selected"</c:if>>Select Labor Type</option>
									<option value="Supply"
						...
						<tr>
							...
							<td width=33%><input id="value(doj)" name="value(doj)"
								onchange="getExp(); setDataChanged(true);" type=text size=20
						...
						<td><html:text property="value(exp)" styleId="exp" readonly="true"
							size="5" maxlength="10" /></td>
						</tr>
						
					<tr>
						<th><label for="khtmlNameInput"><span>*</span>
							<fmt:message key="lable.prevExperience" /></label></th>
						<td ><html:text property="value(prevExp)" styleId="prevExp" 
						onchange="setDataChanged();" size="10" maxlength="10" /></td>
					</tr>
					<tr>
						<th><label for="khtmlNameInput"><span>*</span>
							<fmt:message key="lable.prevOrganization" /></label></th>
						...
						<tr>
							...
							<td width=33%><input id="value(dot)" name="value(dot)"
								onchange="validateDOT(); setDataChanged();" type=text size=20 
					...
					<tr>
						...
						<tr>
							...
							<td><html:text property="value(pVillage)"
									styleId="pVillage" onchange="setDataChanged();" size="40"
									maxlength="50" /></td>
						</tr>
						<tr>
							<th><label for="khtmlNameInput">
							...
							<td><html:text property="value(pTaluka)"
									styleId="pTaluka" onchange="setDataChanged();" size="40"
									maxlength="50" /></td>
						</tr>
						<tr>
							<th><label for="khtmlNameInput">
							...
							<td><html:text property="value(pDistrict)"
									styleId="pDistrict" onchange="setDataChanged();" size="40"
									maxlength="50" /></td>
						</tr>
						...
						<tr>
							<th><label for="khtmlNameInput"><span><font color="darkblue"><fmt:message
										key="label.same.address.checkbox" /></span></label></th>
							<td><html:checkbox property="value(address)"
									styleId="address" onchange="sameAddress();"/></td>
						</tr>
						...
						<tr>
							...
							<td><html:text property="value(permVillage)"
									styleId="permVillage" onchange="setDataChanged();" size="40"
									maxlength="50" /></td>
						</tr>
						<tr>
							<th><label for="khtmlNameInput">
							...
							<td><html:text property="value(permTaluka)"
									styleId="permTaluka" onchange="setDataChanged();" size="40"
									maxlength="50" /></td>
						</tr>
						<tr>
							<th><label for="khtmlNameInput">
							...
							<td><html:text property="value(permDistrict)"
									styleId="permDistrict" onchange="setDataChanged();" size="40"
									maxlength="50" /></td>
						</tr>
						...
						<tr>
							...
							<%-- <td><html:text property="value(bloodgroup)"
									styleId="khtmlNameInput" onchange="setDataChanged();" /></td> --%>
							<td>
								...
								name="value(deviceGroupId)">
									...
									<c:forEach var="deviceGroup" items="${availDeviceGroupNames}">
										<option value="<c:out value="${deviceGroup.deviceGroupId}"/>"
											<c:if test="${deviceGroup.deviceGroupId == devId}">selected</c:if>>
						...
						<tr>
							<th><label for="khtmlNameInput">Medical Check-up Done?</label></th>
						...
						<tr>
							...
							<td><html:text property="value(aadharno)"
									styleId="khtmlNameInput" onchange="setDataChanged();" size="12"
									maxlength="12"/></td>
						</tr>
						<tr>
						...
						<tr>
							<th><label for="khtmlNameInput">Husband/Wife Name</label></th>
						...
						<tr>
							...
							<th><html:text property="value(proflic1)"
									styleId="khtmlNameInput" onchange="setDataChanged();" /></th>
						...
						<tr>
							...
							<td><html:text property="value(vehicleRent)"
									styleId="vehicleRent" onchange="setDataChanged();" /></td>
						</tr>
						...
						<tr>
							...
							<td><html:text property="value(quarterRent)"
									styleId="quarterRent" onchange="setDataChanged();" /></td>
						</tr>
						...
						<tr>
							...
							<td><html:textarea property="value(relativeAddr)"
									styleId="khtmlNameInput" onchange="setDataChanged();" cols="35"
									rows="7" /></td>
						</tr>
						<tr>
							<th><label for="khtmlNameInput"><span></span>
						...
						<tr>
							<th><label for="khtmlNameInput">Is land loser</label></th>
							<td><html:checkbox property="value(isLandLoser)"
									styleId="khtmlNameInput" onchange="setDataChanged();" /></td>
						</tr>
						<tr>
							<th><label for="khtmlNameInput"><fmt:message
