...
function reset(){
    ...
    document.forms.lrbulkForm.doAction ('<%= KronosProperties.get("cms.action.laborreq.refresh").replaceAll("'","\\\\'") %>');
}
function getLRS() {
	   if(checkUnsavedData()){
...
<bean:define id="fromDtm" name="lrbulkForm" property="value(fromDtm)"/>
<bean:define id="toDtm" name="lrbulkForm" property="value(toDtm)"/>
<bean:define id="statusList" name="lrbulkForm" property="value(availStatus)"/>
    ...
    <c:set var="rowIndex" value="${0}"/>
	 ...
	 <c:forEach var = "item" items = "${webServiceConfigTable}" varStatus="status">					
		...
		<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
			<td>
				<input type="checkbox" class="Selected" name="valueAsStringArray(selectedIds)" value="<c:out value='${item.pageId}' />" <c:out value='${item.checked}'  /> onclick="TableRowsSelection.doSelectRow(this)" />
			</td>
			<td >
			<c:if test="${item.pageNum == null}">
				&nbsp;
			</c:if>
				<a href="javascript:setSelectedItem(<c:out value="${item.pageId}" />);document.forms.lrbulkForm.doAction('<%= KronosProperties.get("cms.action.laborreq.view.lr").replaceAll("'","\\\\'") %>')"><c:out value='${item.pageDispNum}'/></a>
