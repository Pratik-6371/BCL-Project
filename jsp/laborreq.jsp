...
<bean:define id="lrLines" name="laborReqForm" property="valueAsList(lrLines)"/>
<bean:define id="wlLines" name="laborReqForm" property="valueAsList(wlLines)"/>
<bean:define id="availShifts" name="laborReqForm" property="value(availShifts)"/>
<bean:define id="tradeList" name="laborReqForm" property="value(tradeList)"/>
<bean:define id="skillList" name="laborReqForm" property="value(skillList)"/>
<bean:define id="fromdtm" name="laborReqForm" property="value(fromdtm)"/>
<bean:define id="todtm" name="laborReqForm" property="value(todtm)"/>
 	 ...
 	 <th colspan=8> <fmt:message key="label.lr.req" />
				...
				<td width=20%>
						...
						<input id="value(fromdtm)"
										name="value(fromdtm)"
										type=text size=10 value="<c:out value="${fromdtm}"/>">
						...
						<input id="value(todtm)" 
										name="value(todtm)" 
										type=text size=10 value="<c:out value="${todtm}"/>">
										</input><kvl:date-selector-eot-bot-popup 
										id="value(todtm)" 
										eot_bot_enable="bot" 
										start_of_week="1" 
    ...
    <c:set var="rowIndex" value="${0}"/>
	 ...
	 <c:forEach var = "item" items = "${lrLines}" varStatus="status">					
		...
		<tr class="last-child <c:if test='${rowIndex % 2 !=  0}'>Even</c:if>">
				<td>
					<kvl:icon id="cms.label.addRow" className="RowInsertIcon" >
						<jsp:attribute name="onclick">(document.getElementsByName('value(selectedId)')[0]).value = '<c:out value="${item.lrId}"/>';document.forms.laborReqForm.doAction('insertRow')
						</jsp:attribute>
					</kvl:icon>
				</td>
			...
			<input  name="value(<c:out value="${item.lrId}"/>_itemDesc)" value="<c:out value="${item.itemDesc}"/>"
				type = "hidden" styleId="khtmlNameInput" onchange="setDataChanged();" size=50 maxlength="50"/>
			<input  name="value(<c:out value="${item.lrId}"/>_wlId)" value="<c:out value="${item.wlId}"/>"
				type = "hidden" styleId="khtmlNameInput" onchange="setDataChanged();" size=50 maxlength="50"/>	
					
			</c:when>
			<c:otherwise>
			<select name="value(<c:out value="${item.lrId}"/>_wlId)" onchange="setValues();"> 
					    <option value="-1"><kvl:label id="cms.label.job"/></option>
						<c:forEach var="wls" items="${wlLines}">
				        <option value="<c:out value="${wls.wkLineId}"/>"
						 <c:if test="${wls.wkLineId == item.wlId}">selected</c:if>
						>
				          <c:out value="${wls.itemDesc}"/>
				        </option>
				        </c:forEach>
					  </select>
			</c:otherwise>
		...
		<td class="last-child">
			...
			<input name="value(<c:out value="${item.lrId}"/>_tradeId)" value="<c:out value="${item.tradeId}"/>"
				type = "hidden" styleId="khtmlNameInput" onchange="setDataChanged();" size=20 maxlength="20" /> 
				
			</c:when>
			<c:otherwise>
			<select name="value(<c:out value="${item.lrId}"/>_tradeId)"> 
					    <option value="-1"><kvl:label id="cms.label.selectTrade"/></option>
						<c:forEach var="trades" items="${tradeList}">
				        <option value="<c:out value="${trades.tradeId}"/>"
						 <c:if test="${trades.tradeId == item.tradeId}">selected</c:if>
						>
				          <c:out value="${trades.tradeName}"/>
				        </option>
				        </c:forEach>
					  </select>
			...
			<input name="value(<c:out value="${item.lrId}"/>_skillId)" value="<c:out value="${item.skillId}"/>"
						type="hidden" styleId="khtmlNameInput" onchange="setDataChanged();" size=20 maxlength="20" /> 
				
			</c:when>
			<c:otherwise>
			<select name="value(<c:out value="${item.lrId}"/>_skillId)"> 
					    <option value="-1"><kvl:label id="cms.label.selectSkill"/></option>
						<c:forEach var="skills" items="${skillList}">
				        <option value="<c:out value="${skills.skillId}"/>"
						 <c:if test="${skills.skillId== item.skillId}">selected</c:if>
						>
				          <c:out value="${skills.skillNm}"/>
				        </option>
				        </c:forEach>
					  </select>
			</c:otherwise>
...
<!-- 				&nbsp; -->
<%-- 			</c:if> --%>
<!-- 				<input 	id ="value(fromdtm)" name="value(fromdtm)" readonly="true" -->
<!-- 						onchange="setDataChanged(true);"  -->
						
<%-- 						type=text size=20 value="<c:out value="${item.from}"/>"> --%>
<%-- 						</input><kvl:date-selector-eot-bot-popup  --%>
<%-- 								id="value(fromdtm)"  --%>
<%-- 								eot_bot_enable="bot"  --%>
<%-- 								start_of_week="1"  --%>
<%-- 								hide_text_field="true"  --%>
<%-- 								text_label_field="label.to" /> --%>
<!-- 			</td> -->
<!-- 			<td class="last-child"> -->
...
<!-- 						readonly="true" -->
<!-- 						onchange="setDataChanged(true);"  -->
<%-- 						type=text size=20 value="<c:out value="${item.to}"/>"> --%>
<%-- 						</input><kvl:date-selector-eot-bot-popup  --%>
<%-- 								id="value(todtm)"  --%>
<%-- 								eot_bot_enable="bot"  --%>
<%-- 								start_of_week="1"  --%>
...
<!-- 			</td> -->
			...
			<c:if test="${item.shift_A== null}">
				...
				<input name="value(<c:out value="${item.lrId}"/>_shift_A)" value="<c:out value="${item.shift_A}"/>"
									styleId="khtmlNameInput" onchange="setDataChanged();" size=3 maxlength="4"/>
				<input name="value(<c:out value="${item.lrId}"/>_shift_A_id)" value="<c:out value="${item.shift_A_id}"/>"
									styleId="khtmlNameInput" type="hidden" />									
			...
			<c:if test="${item.shift_B == null}">
				...
				<input name="value(<c:out value="${item.lrId}"/>_shift_B)" value="<c:out value="${item.shift_B}"/>"
									styleId="khtmlNameInput" onchange="setDataChanged();" size=3 maxlength="4"/>
				<input name="value(<c:out value="${item.lrId}"/>_shift_B_id)" value="<c:out value="${item.shift_B_id}"/>"
									styleId="khtmlNameInput" type="hidden" />									
			...
			<c:if test="${item.shift_C == null}">
				...
				<input name="value(<c:out value="${item.lrId}"/>_shift_C)" value="<c:out value="${item.shift_C}"/>"
									styleId="khtmlNameInput" onchange="setDataChanged();" size=3 maxlength="4"/>
				<input name="value(<c:out value="${item.lrId}"/>_shift_C_id)" value="<c:out value="${item.shift_C_id}"/>"
									styleId="khtmlNameInput" type="hidden" />									
			</td>
			<td class="last-child">
			<c:if test="${item.shift_G == null}">
				...
				<input name="value(<c:out value="${item.lrId}"/>_shift_G)" value="<c:out value="${item.shift_G}"/>"
									styleId="khtmlNameInput" onchange="setDataChanged();" size=3 maxlength="4"/>
				<input name="value(<c:out value="${item.lrId}"/>_shift_G_id)" value="<c:out value="${item.shift_G_id}"/>"
									styleId="khtmlNameInput" type="hidden" />
